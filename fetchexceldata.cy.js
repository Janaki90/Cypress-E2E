...
describe('Fetch Data from Excel', () => {
    ...
    it("should perform the datadriven fetch from excel for multiple data", () => {
        ...
        cy.task('parseXlsx',{ filePath }, sheetName).then((data) => {
            const firstSheet = data[0]; // Get the first sheet
            const headers = firstSheet.data[0]; // First row is the header
    ...
    it("should perform the datadriven fetch from excel directly", () => {
        ...
        const excelFilePath = "cypress/fixtures/rediff_excel.xlsx";
        const sheetName = "Details";

        cy.task('parseXlsx', {excelFilePath} , sheetName).then((data) => {
            const headers = data[0]; // First row (header row)
            const rowData = data[1]; // Second row (first data row)
    ...
    it("should perform the datadriven fetch from excel for contact sales", () => {
        ...
        const excelFilePath = "cypress/fixtures/orange_excel.xlsx";
        const sheetName = "Details";
